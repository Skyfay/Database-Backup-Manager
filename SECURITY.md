...
- [ ] **Secure Adapter API (`src/app/api/adapters/route.ts`)**
    - [ ] Add `auth.api.getSession` check at the beginning of `GET` handler.
    - [ ] Add `auth.api.getSession` check at the beginning of `POST` handler.
    - [ ] Return `401 Unauthorized` if session is missing.

- [ ] **Audit & Secure All Other API Routes**
    - [ ] Check `src/app/api/jobs/`
    - [ ] Check `src/app/api/history/`
    - [ ] Check `src/app/api/storage/`
    - [ ] Apply the same authentication checks as above.

- [ ] **Fix RCE Vulnerabilities in Adapters**
    - [ ] Refactor `PostgresAdapter` (`src/lib/adapters/database/postgres.ts`) to use `child_process.execFile` or strictly validate/sanitize `config.options`.
    - [ ] Refactor `MysqlAdapter` (if applicable).
    - [ ] Refactor `MongodbAdapter` (if applicable).
    - [ ] **Constraint:** Ensure `config.options` only accepts safe flags or remove the ability to pass raw options entirely if possible.

### Structural Improvements (Medium Priority)

- [ ] **Implement Global Middleware**
    - [ ] Create `src/middleware.ts`.
    - [ ] Configure `better-auth` middleware or manual session check to protect `/dashboard/*` and `/api/*` routes by default.
    - [ ] Exclude public routes (e.g., `/`, `/api/auth/*`, `/login`).
